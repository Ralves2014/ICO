...
% RA FUNÇÕES f DE 2 VARIÁVEIS (NVAR=1).
...
% - f: FUNÇÃO SOB O PROCESSO ITERATIVO DO MÉTODO LS USADO
...
% graficosLinesearchNvar1. O NOME É SEGUIDO DE UM PARÊNTESIS ONDE CONSTAM 
% OS INPUTS, QUE NESTE CASO SÃO Lista, LNit E Lopt.
...
for i=1:length(LNit)  % INDEXAÇÃO POR i DOS PTOS INICIAIS USADOS NO PROCES-
                      ...
                      % COMPRIMENTO length(LNit) DA LISTA DE ARQUIVO LNit
    Nfinal=Ninicial+LNit(i)-1;% CONTAGEM DO Nº FINAL (Nfinal) DE ITERAÇOES
                               % OBTIDAS DO i-ÉSIMO PTO INICIAL. EXEMPLO:
                               % QUANDO Ninicial=1 ENTÃO LNit(2)=3 SIGNIFI-
                               % CA QUE O PTO INICIAL i=2 GEROU UM PROCESSO
        ...
        Ly=[];                 % LY: CRIA ARQUIVO VAZIO 
        for j=1:length(Lx)     % INDEXAÇÃO POR j DAS ABCISSAS DO ARQUIVO Lx, 
                               % SENDO O Nº DE ABCISSAS IGUAL AO COMPRIMEN-
                               % TO length(lx) DO ARQUIVO lx
        Ly=[Ly f(Lx(j))];      % OBTER A LISTA Ly (MATRIZ C/ 2 COLUNAS) DAS 
                               % ORDENADAS f(x) DOS PTOS DA SEQUÊNCIA GERA-
                               % DA A PARTIR DO i-ÉSIMO PTO INICIAL x
